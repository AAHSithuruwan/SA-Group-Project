...
const SellerAuctionList = () => {
  ...
  const handleDetailsClick = (auction) => {
    navigate('/sellerauctionlist/sellerauctiondetails', { state: { auctionId: auction.auctionId } }); 
  };
  ...
  useEffect(() => {
    const fetchSellerAuctions = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5101/api/Auction/Seller`,{
          headers: {
        ...
        console.error('There was an error fetching the seller auctions!', error);
      }
    };
